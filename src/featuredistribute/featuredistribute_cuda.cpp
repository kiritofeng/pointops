...
void featuregather_forward_cuda(int b, int n, int m, int c,
  ...
  featuregather_forward_cuda_launcher(b, n, m, c, max_feature, distribute_idx,
...
void featuregather_backward_cuda(int b, int n, int m, int c,
  ...
  const at::cuda::OptionalCUDAGuard device_guard(
      device_of(grad_distribute_feature_tensor));

  const float *grad_distribute_feature =
      grad_distribute_feature_tensor.data_ptr<float>();
  const int *distribute_idx = distribute_idx_tensor.data_ptr<int>();
  float *grad_max_feature = grad_max_feature_tensor.data_ptr<float>();

  cudaStream_t stream = at::cuda::getCurrentCUDAStream();

  featuregather_backward_cuda_launcher(b, n, m, c, grad_distribute_feature,
                                       distribute_idx, grad_max_feature,
                                       stream);
}
