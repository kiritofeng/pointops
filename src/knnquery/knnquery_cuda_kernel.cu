...
__global__ void knnquery_cuda_kernel(int b, int n, int m, int nsample,
...
void knnquery_cuda_launcher(int b, int n, int m, int nsample, const float *xyz,
                            const float *new_xyz, int *idx, float *dist2,
                            cudaStream_t stream) {
  // param new_xyz: (B, m, 3)
  // param xyz: (B, n, 3)
  ...
  knnquery_cuda_kernel<<<blocks, threads, 0, stream>>>(b, n, m, nsample, xyz,
                                                       new_xyz, idx, dist2);
